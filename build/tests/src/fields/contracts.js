...
export const FIELD_CONTRACTS = {
    rim: {
        ...
        format: "float32",
        channels: [
            { id: "gradX", semantic: "gradientX", description: CHANNEL_DESCRIPTIONS.gradientX, components: 1 },
            { id: "gradY", semantic: "gradientY", description: CHANNEL_DESCRIPTIONS.gradientY, components: 1 },
            { id: "mag", semantic: "magnitude", description: CHANNEL_DESCRIPTIONS.magnitude, components: 1 }
        ],
        lifetime: {
            kind: "static",
    ...
    phase: {
        ...
        format: "float32",
        channels: [
            { id: "gradX", semantic: "phaseGradX", description: CHANNEL_DESCRIPTIONS.phaseGradX, components: 1 },
            { id: "gradY", semantic: "phaseGradY", description: CHANNEL_DESCRIPTIONS.phaseGradY, components: 1 },
            { id: "vort", semantic: "phaseVorticity", description: CHANNEL_DESCRIPTIONS.phaseVorticity, components: 1 },
            { id: "coh", semantic: "phaseCoherence", description: CHANNEL_DESCRIPTIONS.phaseCoherence, components: 1 }
        ],
        lifetime: {
            kind: "dynamic",
            expectedMs: 16,
    ...
    volume: {
        ...
        format: "float32",
        channels: [
            { id: "phase", semantic: "volumePhase", description: CHANNEL_DESCRIPTIONS.volumePhase, components: 1 },
            { id: "depth", semantic: "volumeDepth", description: CHANNEL_DESCRIPTIONS.volumeDepth, components: 1 },
            { id: "intensity", semantic: "volumeIntensity", description: CHANNEL_DESCRIPTIONS.volumeIntensity, components: 1 }
        ],
        lifetime: {
            kind: "workerStream",
            expectedMs: 16,
...
const uppercaseSnake = (value) => value.replace(/([a-z0-9])([A-Z])/g, "$1_$2").replace(/[-\s]+/g, "_").toUpperCase();
const buildChannelDefines = () => Object.entries(CHANNEL_SEMANTICS)
    .map(([key, code]) => `#define FIELD_SEMANTIC_${uppercaseSnake(key)} ${code}`)
    .join("\n");
const buildKindDefines = () => FIELD_KIND_LIST.map((kind) => `#define FIELD_KIND_${uppercaseSnake(kind)} ${FIELD_KIND_CODES[kind]}`).join("\n");
const buildStorageDefines = () => Object.entries(FIELD_STORAGE)
    .map(([key, code]) => `#define FIELD_STORAGE_${uppercaseSnake(key)} ${code}`)
    .join("\n");
const buildFormatDefines = () => Object.entries(FIELD_COMPONENT_FORMAT)
    .map(([key, code]) => `#define FIELD_FORMAT_${uppercaseSnake(key)} ${code}`)
    .join("\n");
const buildLifetimeDefines = () => Object.entries(FIELD_LIFETIME_KIND)
...
const padChannels = (channels) => {
    const fallback = { semantic: -1, components: 0, format: FIELD_COMPONENT_FORMAT.float32 };
    return [
        channels[0] ?? fallback,
        channels[1] ?? fallback,
        channels[2] ?? fallback,
...
export const makeFieldUniformSnapshot = (contract, runtime) => {
    const resolution = runtime.available && runtime.resolution
        ? [runtime.resolution.width, runtime.resolution.height]
        : [0, 0];
    const channels = padChannels(contract.channels.map((descriptor) => ({
...
const assertLength = (buffer, expected, label, field, source) => {
    assert(buffer.length === expected, `[fields:${field}] ${label} length ${buffer.length} != expected ${expected} (${source})`);
};
