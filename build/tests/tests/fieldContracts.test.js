...
import { assertPhaseField, assertRimField, assertSurfaceField, assertVolumeField, makeResolution, } from '../src/fields/contracts.js';
import { createInitialStatuses, markFieldUnavailable, markFieldUpdate, refreshFieldStaleness, } from '../src/fields/state.js';
...
const makeVolumeField = (width, height) => ({
    kind: 'volume',
    resolution: makeResolution(width, height),
...
test('assertRimField rejects mismatched buffers', () => {
    ...
    assert.throws(() => assertRimField(field, 'test'));
});
test('surface field guard enforces rgba length', () => {
    ...
    const bad = { ...good, rgba: new Uint8ClampedArray(3) };
...
test('phase and volume assertions verify channels', () => {
    ...
    const brokenPhase = { ...phase, vort: new Float32Array(2) };
    ...
    const brokenVolume = { ...volume, intensity: new Float32Array(1) };
...
test('field staleness lifecycle', () => {
    ...
    assert.ok(changes.some((change) => change.kind === 'phase' && change.becameStale));
