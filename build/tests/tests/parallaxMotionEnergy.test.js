...
const createRimField = (width, height) => {
...
const createRadialPhaseField = (width, height) => {
    const total = width * height;
...
test("motion energy captures cross-branch phase shift for multi-orientation overlays", () => {
    ...
    assert.ok(motionEnergy.phaseShiftMean > 0.05, `phase shift too small (${motionEnergy.phaseShiftMean})`);
});
