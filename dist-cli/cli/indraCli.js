...
const handleManifestCommand = async (args) => {
...
const handleApplyCommand = async (args) => {
...
const handleSimulateCommand = async (args) => {
  if (args.includes('--help') || args.includes('-h')) {
    printSimulateUsage();
    process.exit(0);
  }
...
const handleCaptureCommand = async (args) => {
  if (args.includes('--help') || args.includes('-h')) {
    printCaptureUsage();
    process.exit(0);
  }
...
const runTelemetryServer = async (args) => {
  let port = 8090;
  let outputPath = null;
