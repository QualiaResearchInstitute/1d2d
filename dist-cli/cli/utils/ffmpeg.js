...
export const probeMedia = async (ffprobe, input) => {
  const args = [
    '-v',
    'error',
    '-select_streams',
  ...
  const payload = JSON.parse(utf8.decode(stdout));
...
export const decodeFrame = async (ffmpeg, input, width, height, options = {}) => {
  const args = ['-v', 'error'];
  if (
...
export const encodeImage = async (ffmpeg, input, width, height, outputPath, options = {}) => {
