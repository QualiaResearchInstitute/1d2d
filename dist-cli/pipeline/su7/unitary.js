...
const buildRandomComplexMatrix = (seed) => {
...
const columnVector = (matrix, column) => matrix.map((row) => ({ ...row[column] }));
...
const normalizeVector = (vector) => {
  ...
  if (norm <= EPSILON) {
    const fallback = new Array(DIM).fill(null).map((_, idx) => ({ re: idx === 0 ? 1 : 0, im: 0 }));
    return fallback;
  }
...
const enforceSpecialUnitary = (matrix) => {
...
const buildRaisingPulse = (pair, theta) => {
  const [i, j] = pair;
  const result = createIdentity();
...
const resolvePresetMatrix = (preset, seed, theta) => {
  if (preset === 'identity') {
...
const composeSchedule = (base, schedule, seed, fallbackTheta) => {
  if (!schedule.length) {
    return base;
  }
  let current = base;
  schedule.forEach((stage, index) => {
    const theta = Number.isFinite(stage.theta) ? stage.theta : fallbackTheta;
...
export const applySU7 = (unitary, vector) => {
...
export const buildUnitary = (options = {}) => {
  const seed = Number.isFinite(options.seed) ? options.seed : DEFAULT_SEED;
  const preset = options.preset ?? DEFAULT_PRESET;
  const fallbackTheta =
    Number.isFinite(options.theta) && options.theta != null ? options.theta : DEFAULT_THETA;
...
export const projectToSU7 = (matrix) => enforceSpecialUnitary(matrix);
