...
import { renderRainbowFrame } from '../pipeline/rainbowFrame.js';
import {
  createKuramotoState,
  createDerivedViews,
...
const LAMBDAS = { L: 560, M: 530, S: 420 };
export const resolveRuntimeConfig = async (manifestPath, presetId) => {
  if (!manifestPath) {
...
const cloneComposer = (config, frame) => ({
  ...config.composer,
  fields: {
    surface: {
...
const buildPhaseFieldFromKuramoto = (derived, width, height) => ({
  kind: 'phase',
...
export const applyFrame = async (options) => {
  ...
  const phaseField = (config.frame.kurEnabled && pipeline.kuramoto?.phase) || pipeline.phase.field;
  const surface = {
    kind: 'surface',
...
export const simulate = async (options) => {
  const probe = await probeMedia(options.ffprobe, options.input);
  const decoded = await decodeFrame(options.ffmpeg, options.input, probe.width, probe.height);
  const pixelData = Uint8ClampedArray.from(decoded);
  const image = { data: pixelData, width: probe.width, height: probe.height };
  const { config, manifestPath, presetId } = await resolveRuntimeConfig(
    options.manifest,
    options.preset,
  );
...
const simulateFramesInternal = async (config, image, options) => {
...
export const captureVideo = async (options) => {
  ...
  try {
    ...
    for (let index = 0; index < frames.length; index++) {
      ...
      const rim = pipeline.rim;
      const phase = (config.frame.kurEnabled && pipeline.kuramoto?.phase) || pipeline.phase.field;
