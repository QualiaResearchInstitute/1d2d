...
const buildKindDefines = () =>
  FIELD_KIND_LIST.map((kind) => `#define FIELD_KIND_${uppercaseSnake(kind)} ${FIELD_KIND_CODES[kind]}`).join("\n");
...
const padChannels = (channels: FieldChannelUniform[]): [
  FieldChannelUniform,
  FieldChannelUniform,
  FieldChannelUniform,
  FieldChannelUniform
] => {
  const fallback: FieldChannelUniform = { semantic: -1, components: 0, format: FIELD_COMPONENT_FORMAT.float32 };
  return [
    channels[0] ?? fallback,
    channels[1] ?? fallback,
    channels[2] ?? fallback,
...
export const makeFieldUniformSnapshot = (
  ...
  const resolution: [number, number] = runtime.available && runtime.resolution
