import { makeResolution, type FieldResolution } from '../fields/contracts.js';

const EPS = 1e-6;
const MAX_LIMIT = 0.999999;

export type HyperbolicMode = 'poincare' | 'klein';

export type HyperbolicAtlasParams = {
  width: number;
  height: number;
...
export type HyperbolicAtlas = {
  resolution: FieldResolution;
  coords: Float32Array;
  polar: Float32Array;
  jacobians: Float32Array;
  areaWeights: Float32Array;
  metadata: HyperbolicAtlasMetadata;
};

export type HyperbolicAtlasMetadata = {
  curvatureStrength: number;
...
export type HyperbolicAtlasGpuPackage = {
  width: number;
  height: number;
  buffer: Float32Array;
  layout: {
...
export const createHyperbolicAtlas = (params: HyperbolicAtlasParams): HyperbolicAtlas => {
  ...
  const defaultCenter = (size: number) => Math.floor(size / 2);
  ...
  for (let y = 0; y < height; y++) {
    for (let x = 0; x < width; x++) {
      ...
      let dDiskX_dNx: number;
      let dDiskX_dNy: number;
      let dDiskY_dNx: number;
      let dDiskY_dNy: number;
...
export const packageHyperbolicAtlasForGpu = (atlas: HyperbolicAtlas): HyperbolicAtlasGpuPackage => {
...
export const mapHyperbolicPolarToPixel = (
  atlas: HyperbolicAtlas,
  radius: number,
  theta: number,
): [number, number] => {
