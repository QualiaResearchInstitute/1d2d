...
import type { C7Vector, Complex } from './types.js';
import { rgbToLms, srgbToLinear } from '../colorSpaces.js';

export type GaugeMode = 'rim' | 'phase' | 'none';

export type EmbedToC7Params = {
  ...
  height?: number;
  gauge?: GaugeMode;
  rimPhaseOffsets?: readonly [number, number, number];
};

export type EmbedToC7Result = {
  vectors: C7Vector[];
  norms: Float32Array;
  width: number;
  height: number;
...
const DEFAULT_PHASE_OFFSETS: readonly [number, number, number] = [
  0,
  (2 * Math.PI) / 3,
  (4 * Math.PI) / 3,
] as const;

const DEFAULT_FALLBACK_VECTOR: C7Vector = [
  { re: 1, im: 0 },
  { re: 0, im: 0 },
  { re: 0, im: 0 },
  { re: 0, im: 0 },
...
const ensureFinite = (value: number): number => (Number.isFinite(value) ? value : 0);

const cloneFallbackVector = (): C7Vector =>
  DEFAULT_FALLBACK_VECTOR.map((entry) => ({ ...entry })) as C7Vector;

const resolveDimensions = (params: EmbedToC7Params): { width: number; height: number } => {
...
const computeGaugeAngle = (
  mode: GaugeMode,
  index: number,
  rim: RimField | null | undefined,
  phase: PhaseField | null | undefined,
): number => {
  const pickPhaseAngle = (): number | null => {
  ...
  const pickRimAngle = (): number | null => {
...
const rotateByGauge = (
  ...
  cosGauge: number,
  sinGauge: number,
): [number, number] => [re * cosGauge + im * sinGauge, im * cosGauge - re * sinGauge];

const buildComplex = (re: number, im: number): Complex => ({
  re,
  im,
});

export const embedToC7 = (params: EmbedToC7Params): EmbedToC7Result => {
  ...
  const vectors: C7Vector[] = new Array(texels);
