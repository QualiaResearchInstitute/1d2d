...
export type TextureDiagnosticsResult = {
  wallpapericityMean: number;
  wallpapericityStd: number;
  beatEnergyMean: number;
  beatEnergyStd: number;
  ...
  divisiveMean: number;
  divisiveStd: number;
  wallpapericityMap?: Float32Array;
  beatEnvelopeMap?: Float32Array;
};
...
const DEFAULT_RESULT: TextureDiagnosticsResult = {
...
const buildGaussianKernel = (sigma: number): Float32Array => {
...
const computeGradients = (gray: Float32Array, width: number, height: number) => {
  const gx = new Float32Array(width * height);
  const gy = new Float32Array(width * height);
  const idx = (ix: number, iy: number) => iy * width + ix;
...
const stats = (sum: number, sumSq: number, count: number) => {
  if (count <= 0) {
    return { mean: 0, std: 0 };
  }
