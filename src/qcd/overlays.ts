...
export type FluxOverlayFrameData = {
  width: number;
  height: number;
  energy: Float32Array;
...
const gaussian = (distance: number, sigma: number): number => {
...
const toPairs = (sources: readonly FluxSource[]): FluxPair[] => {
  const positives: FluxSource[] = [];
  const negatives: FluxSource[] = [];
  ...
  const pairs: FluxPair[] = [];
  const negTaken = new Set<number>();
...
export const buildFluxSu3Block = (normalizedEnergy: number, angle: number): Complex3x3 => {
...
export const fluxEnergyToOverlayColor = (
