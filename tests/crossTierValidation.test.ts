...
test('cross-tier validation estimates tracer half-life close to configured targets', () => {
  ...
  for (const tier of TIERS) {
    const diff = Math.abs(
      report.baseline[tier].measuredHalfLife - report.baseline[tier].expectedHalfLife,
    );
    assert.ok(diff < 0.12, `${tier} baseline half-life diff ${diff.toFixed(3)} exceeds tolerance`);
    const variantDiff = Math.abs(
      report.variant[tier].measuredHalfLife - report.variant[tier].expectedHalfLife,
    );
    assert.ok(
      variantDiff < 0.12,
      `${tier} variant half-life diff ${variantDiff.toFixed(3)} exceeds tolerance`,
    );
  }
});
...
test('cross-tier validation records kernel-induced deltas across tiers', () => {
  ...
  for (const tier of TIERS) {
    ...
    assert.ok(
      Math.abs(delta) > 1e-4,
      `${tier} expected measurable kernel delta magnitude, received ${delta.toFixed(4)}`,
    );
  }
});
...
test('cross-tier validation raises coherence alerts when noise exceeds tolerance', () => {
  ...
  const summaries = summarizeAlerts(coherenceAlerts as CrossTierAlert[]);
  assert.ok(
    summaries.some((line) => line.includes('coherence drop')),
    'summaries should include coherence drop message',
  );
});
